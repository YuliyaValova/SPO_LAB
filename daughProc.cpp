...
int main(int argc, char* argv[]) {
	...
	for (int i = 0; i < kol; i++) {
			sportsmens[i].printConsole();
			WriteFile(file, &sportsmens[i].comandName, sizeof(sportsmens[i].comandName), &n, NULL);
			WriteFile(file, &sportsmens[i].fio, sizeof(sportsmens[i].fio), &n, NULL);
			WriteFile(file, &sportsmens[i].age, sizeof(sportsmens[i].age), &n, NULL);
	};
